...
public class WordCount3 extends Configured implements Tool {
  ...
  public int run(String[] args) throws Exception {
    Job job = Job.getInstance(getConf(), "WordCount3");
    
    for (int i = 0; i < args.length; i += 1) {
      if ("-skip".equals(args[i])) {
        job.getConfiguration().setBoolean("WordCount3.skip.patterns", true);
        i += 1;
  ...
  public static class Map extends Mapper<LongWritable, Text, Text, IntWritable> {
    ...
    protected void setup(Mapper.Context context)
      ...
      this.caseSensitive = config.getBoolean("WordCount3.case.sensitive", false);
      if (config.getBoolean("WordCount3.skip.patterns", false)) {
